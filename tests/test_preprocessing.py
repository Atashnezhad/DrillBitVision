...
def test_download_images_2(_object):
    ...
    with mock.patch("neural_network_model.process_data.downloader") as mock_bing_downloader:
        _object.download_images()
        print(mock_bing_downloader.download.call_count)
        assert mock_bing_downloader.download.call_count == 2
...
def test_download_images_3(mocker, _object):
    mock_bing_downloader_download = mocker.patch("neural_network_model.process_data.downloader.download")
    ...
    assert mock_bing_downloader_download.call_count == 2  # by default, the number of categories to download is 2
...
def test_download_images_4_s3(mocker, _object):
    mock_my_s3_obj = mocker.patch("neural_network_model.process_data.MyS3")
    _object.download_images(from_s3=True)
    # assert mock_my_s3_obj.download_from_s3.call_count == 1
    assert mock_my_s3_obj.download_files_from_subfolders.called is True
